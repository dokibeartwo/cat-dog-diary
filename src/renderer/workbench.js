window.Workbench = (() => {
  ...
  let tab='upcoming', planToken=null, busy=false, category='', settingsInfo=null, syncConflicts=[], syncMergePreview=null, conflictDrafts={};
  ...
  async function settings() {
    ...
    if(!syncInfo.signedIn||!syncInfo.needsMerge)syncMergePreview=null;
    const prefs=ui.state.preferences;
    const syncAccountMarkup = !syncInfo.signedIn
      ? '<div class="dialog-grid"><label>邮箱<input id="syncEmail" type="email" placeholder="name@example.com" /></label><label>验证码<input id="syncOtp" inputmode="numeric" placeholder="邮箱中的验证码" /></label></div><div class="settings-buttons"><button id="syncSendOtp">发送验证码</button><button id="syncVerifyOtp" class="primary-button">验证并登录</button></div>'
      : `<div class="settings-buttons">${syncInfo.needsMerge?'': `<button id="syncNow" ${syncInfo.paused?'disabled':''}>立即同步</button><button id="syncPause" data-paused="${syncInfo.paused?'1':'0'}">${syncInfo.paused?'恢复自动同步':'暂停自动同步'}</button>`}${syncInfo.needsMerge&&!syncMergePreview?'<button id="syncPreviewMerge">预览首次同步</button>':''}<button id="syncLogout">退出账号</button><button id="syncDeleteAccount" class="danger-button">删除云端账号</button></div>`;
    q('settingsContent').innerHTML=`${!prefs.onboardingDone?'<div class="welcome-note"><strong>欢迎使用猫狗日记</strong><p>默认数据只保存在本机。预置习惯均未开启；如需跨设备使用，可在下方自行配置可选的云同步。</p><button id="finishWelcome" class="primary-button">知道了，开始使用</button></div>':''}
      ...
      ${syncAccountMarkup}
      ${syncMergePreview?`<div class="sync-merge-preview"><h4>首次同步：先预览，再合并</h4><p class="field-hint">本机已有 ${syncMergePreview.localEntities} 条可同步记录，云端已有 ${syncMergePreview.cloudEntities} 条。确认前不会改变双方数据。确认后先备份，再合并；同字段冲突由你选择，不自动覆盖或复活删除记录。</p><label>处理方式<select id="syncMergeStrategy"><option value="merge">合并（保留双方内容，冲突单独选择）</option></select></label><div class="settings-buttons"><button id="syncMergeConfirm" class="primary-button">确认并同步</button><button id="syncMergeRefresh">重新预览</button></div></div>`:''}
      ${syncConflicts.length?`<div class="sync-conflicts"><h4>需要你处理的同步冲突（${syncConflicts.length}）</h4><p class="field-hint">同一字段在不同设备被同时修改。选好每个字段后一次确认。删除冲突默认保留云端状态。</p>${syncConflicts.slice(0,20).map(conflict=>`<div class="sync-conflict" data-conflict-id="${e(conflict.conflict_id||conflict.conflictId)}"><strong>${e(conflict.entity_type||conflict.entityType)} · ${e(conflict.entity_id||conflict.entityId)}</strong>${(conflict.conflict_fields||[]).map(field=>{const local=conflict.local_patch?.[field],remote=conflict.remote_payload?.[field];return `<div class="sync-conflict-field"><span>${e(field)}</span><button data-conflict-choice="local" data-conflict-id="${e(conflict.conflict_id||conflict.conflictId)}" data-conflict-field="${e(field)}">本机：${e(typeof local==='object'?JSON.stringify(local):local)}</button><button data-conflict-choice="remote" data-conflict-id="${e(conflict.conflict_id||conflict.conflictId)}" data-conflict-field="${e(field)}">云端：${e(typeof remote==='object'?JSON.stringify(remote):remote)}</button></div>`;}).join('')}<button data-conflict-apply="${e(conflict.conflict_id||conflict.conflictId)}" class="primary-button">确认全部选择</button><button data-conflict-remote="${e(conflict.conflict_id||conflict.conflictId)}">全部保留云端状态</button></div>`).join('')}</div>`:''}</div>
      <h3>数据备份与恢复</h3><p class="field-hint" style="overflow-wrap:anywhere">当前数据文件：${e(settingsInfo.path || '')}${settingsInfo.customDirectory?'<br>使用独立数据目录，不会自动读取或覆盖正式版任务；需要时可导入正式版导出的备份。':''}</p><p class="field-hint">每日首次保存自动备份；恢复前保留原数据。备份含任务正文，请只发给信任的人。删除任务可从下方回收站找回。</p><div class="settings-buttons"><button id="backupNow">立即备份</button><button id="exportData">导出到文件</button><button id="importData">导入备份</button></div>
  ...
  function init() {
    ...
    q('settingsContent').addEventListener('click',event=>{
      ...
      safely(async()=>{
        ...
        if(button.id==='syncVerifyOtp'){ui.state=await window.doneAPI.verifySyncOtp(q('syncEmail').value,q('syncOtp').value);syncMergePreview=ui.state.sync.needsMerge?await window.doneAPI.previewSyncMerge():null;await settings();showToast(syncMergePreview?'已登录，请先确认首次同步方式':'已登录，本机内容保留');}
        if(button.id==='syncPreviewMerge'){syncMergePreview=await window.doneAPI.previewSyncMerge();await settings();}
        if(button.id==='syncMergeRefresh'){syncMergePreview=await window.doneAPI.previewSyncMerge();await settings();}
        if(button.id==='syncMergeConfirm'){const strategy=q('syncMergeStrategy')?.value || 'merge';const result=await window.doneAPI.mergeSyncData(strategy,syncMergePreview?.token);syncMergePreview=null;ui.state=result;await settings();renderAll();showToast(`首次同步完成：${strategy==='cloud'?'已使用云端内容':strategy==='local'?'已保留本机内容':'已合并双方内容'}`);}
        ...
        if(button.id==='syncPause'){await window.doneAPI.pauseSync(button.dataset.paused!=='1');await settings();}
        if(button.id==='syncLogout'){syncMergePreview=null;await window.doneAPI.logoutSync();await settings();showToast('已退出同步账号，本机数据保留');}
        ...
        if(button.dataset.conflictChoice){
          ...
          conflictDrafts[button.dataset.conflictId]={...(conflictDrafts[button.dataset.conflictId]||{}),...resolution};
          button.parentElement.querySelectorAll('button').forEach(b=>b.classList.toggle('primary-button',b===button));
        }
        if(button.dataset.conflictApply||button.dataset.conflictRemote){
          const id=button.dataset.conflictApply||button.dataset.conflictRemote;
          const conflict=syncConflicts.find(item=>String(item.conflict_id||item.conflictId)===String(id));
          if(!conflict)throw Error('冲突已变化，请重新读取');
          const fields=conflictDrafts[id]||{};
          if(!button.dataset.conflictRemote && conflict.conflict_fields.includes('deletedAt'))throw Error('此记录涉及删除。本版请保留云端状态；需要恢复时从回收站恢复。');
          if(!button.dataset.conflictRemote && conflict.conflict_fields.some(field=>!Object.prototype.hasOwnProperty.call(fields,field)))throw Error('请先选择全部冲突字段');
          await window.doneAPI.resolveSyncConflict(id,{expectedRevision:conflict.remote_revision,action:button.dataset.conflictRemote?'keepRemote':'merge',fields});
