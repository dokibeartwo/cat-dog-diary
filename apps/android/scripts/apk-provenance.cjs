...
(async()=>{
  const id=process.env.APK_SOURCE_RUN,repo=process.env.GITHUB_REPOSITORY;
  ...
  if(run.path!=='.github/workflows/android-native.yml'||run.head_repository?.full_name!==repo||run.status!=='completed')throw Error('APK must be from a completed native build in this repository');
